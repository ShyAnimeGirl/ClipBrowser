...
import winreg #you can remove this and other registry related code if you are not on windows or are setting your default in a non standard way
...
try:
	icon_key = winreg.OpenKeyEx(hive, r"SOFTWARE\\ClipBrowser\\CustomIcon\\")
except:
	icon_key = 0
	if(debug):
		print("custom icon not set up")	
		
if icon_key:
	keystore = winreg.QueryValueEx(icon_key, "Enabled")
	if(keystore[0]):
		icon = 1
		keystore = winreg.QueryValueEx(icon_key, "Icon_path")
		icon_path = keystore[0]
...
if(n == 4):
	if((sys.argv[1] == "INSTALL") & (sys.argv[2] == "PATH")):
		...
		send_notif(sys.argv[3], "Installing As:")
		...
		winreg.SetValueEx(current_sub_key, "AppUserModelId", 0, winreg.REG_SZ, "ClipBrowser")
		winreg.CloseKey(current_sub_key)
		...
		register = winreg.OpenKeyEx(hive, r"SOFTWARE\\RegisteredApplications\\", 0, winreg.KEY_ALL_ACCESS)
		winreg.SetValueEx(register, "ClipBrowser", 0, winreg.REG_SZ, "Software\\Clients\\StartMenuInternet\\ClipBrowser\\Capabilities")
		
		#prob redundent
		winreg.CloseKey(software)
		winreg.CloseKey(register)
...
if(n == 4):
	if((sys.argv[1] == "INSTALL") & (sys.argv[2] == "SOUND")):
		send_notif(sys.argv[3], "Setting Custom Sound:")
...
#windows reg 	
if(n == 4):
	if((sys.argv[1] == "INSTALL") & (sys.argv[2] == "ICON")):
		send_notif(sys.argv[3], "Setting Custom Sound:")

		software = winreg.OpenKeyEx(hive, r"SOFTWARE\\")
		current_sub_key = winreg.CreateKey(software, "ClipBrowser\\CustomIcon")
		winreg.SetValueEx(current_sub_key, "Enabled", 0, winreg.REG_DWORD, 1)
		winreg.SetValueEx(current_sub_key, "Icon_path", 0, winreg.REG_SZ, sys.argv[3])
